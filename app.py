...
# ==============================================================================
# 1. إعدادات الصفحة والتصميم العام (Cairo Font & Theme)
# ==============================================================================
st.set_page_config(
    ...
    with get_db_connection() as conn:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            INSERT INTO company VALUES (
                ...
                'مصرف الراجحي', 'SA918000066608016322302', '', '', ''
            )
            """)
        ...
        # تحديث كلمات المرور إلى Smart@2026
...
# ==============================================================================
# 3. محرك توليد PDF والتصميم الكامل المعتمد (مع دعم لينكس السحابي وويندوز)
# ==============================================================================
...
<div style="font-size:0.85rem; font-weight:800; color:#ea580c; letter-spacing:1.5px; margin-top:2px;">SMART FUTURE VISION</div>
    ...
    # 2. الختم الرسمي الدائري المزدوج
...
<div style="font-size:0.68rem; font-weight:bold;">س.ت {comp_data.get('cr_number', '1009160447')}</div>
<div style="font-size:1.3rem; font-weight:900; margin:1px 0;">رؤيــة</div>
<div style="font-size:0.62rem; font-weight:bold;">C.R {comp_data.get('cr_number', '1009160447')}</div>
<div style="font-size:0.58rem; font-weight:bold;">Smart Future Vision Foundation</div>
</div>"""

    # 3. التوقيع الأزرق الرسمي
...
<div style="font-size:0.65rem; font-weight:bold; margin-top:2px;">ختم هيئة الزكاة QR</div>
    ...
    # هيكل A4 بنظام Flexbox لضمان بقاء الفوتر في القاع تماماً
...
<div style="font-size:0.95rem; font-weight:bold;">الرقم الضريبي: <span style="font-family:sans-serif;">{comp_data.get('vat_number', '')}</span></div>
</div>
</div>
...
<div style="font-weight:bold;">اسم المستفيد: {comp_data.get('name', '')}</div>
<div style="font-weight:bold;">اسم البنك: {comp_data.get('bank_name', '')}</div>
<div style="font-weight:bold;">رقم الحساب: <span style="font-family:sans-serif;">{acc_num}</span></div>
<div style="font-weight:bold;">رقم الآيبان: <span style="font-family:sans-serif;">{comp_data.get('iban', '')}</span></div>
</div>
...
<div>{comp_data.get('name', '')} ، السجل التجاري : {comp_data.get('cr_number', '')}</div>
<div>رقم الهاتف : {comp_data.get('phone', '')} &nbsp;&nbsp;|&nbsp;&nbsp; العنوان : {comp_data.get('city', '')} {comp_data.get('district', '')} &nbsp;&nbsp;|&nbsp;&nbsp; {comp_data.get('email', '')}</div>
</div>
</div>"""
    ...
    with col_login:
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("تسجيل الدخول 🚀", type="primary", use_container_width=True):
                with get_db_connection() as conn:
                    user_row = conn.execute("SELECT * FROM users WHERE username=? AND password=?", (u_name.strip().lower(), u_pass.strip())).fetchone()
                    if user_row:
                        st.session_state.current_user = dict(user_row)
                        user_display = user_row['name'] if 'name' in user_row.keys() else u_name
                        st.success(f"مرحباً بك، {user_display}")
                        st.rerun()
                    else:
...
user_role = current_u.get("role", "ADMIN")
user_name = current_u.get("name", "المستخدم")
user_title = current_u.get("role_title", "مدير النظام")
...
with st.sidebar:
    ...
    st.write(f"👤 المستخدم: **{user_name}**")
    
    badge_style = "role-admin" if user_role == "ADMIN" else ("role-accountant" if user_role == "ACCOUNTANT" else "role-sales")
    st.markdown(f"الصلاحية: <span class='role-badge {badge_style}'>{user_title}</span>", unsafe_allow_html=True)
            ...
            for q in quotes:
                with st.expander(f"📌 عرض رقم: {q['id']} — {q['customer_name']} ({q['total']:,.2f} ر.س)"):
                    ...
                    if pdf_bytes:
                        ...
                        c_act1.download_button(
                            label="📥 تحميل العرض للطباعة (HTML/PDF)",
                            data=f"<!DOCTYPE html><html dir='rtl' lang='ar'><head><meta charset='utf-8'></head><body>{doc_html}</body></html>",
                            file_name=f"عرض_سعر_{q['id']}.html",
                            mime="text/html",
                            key=f"dl_q_html_{q['id']}",
                            type="primary",
                            use_container_width=True
                        )
            ...
            for inv in invoices:
                with st.expander(f"🧾 فاتورة: {inv['invoice_number']} — {inv['customer_name']} ({inv['total']:,.2f} ر.س)"):
                    ...
                    if inv_pdf:
                        ...
                        col_b1.download_button(
                            label="📥 تحميل الفاتورة للطباعة (HTML/PDF)",
                            data=f"<!DOCTYPE html><html dir='rtl' lang='ar'><head><meta charset='utf-8'></head><body>{inv_html}</body></html>",
                            file_name=f"فاتورة_{inv['invoice_number']}.html",
                            mime="text/html",
                            key=f"dl_inv_html_{inv['id']}",
                            type="primary",
                            use_container_width=True
                        )
...
# ==============================================================================
    ...
    with st.form("company_info_form"):
        ...
        c_iban = col3.text_input("رقم الآيبان (IBAN)", value=comp.get("iban", "SA918000066608016322302"))
